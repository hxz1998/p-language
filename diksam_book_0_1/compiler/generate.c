...
add_constant_pool(DVM_Executable *exe, DVM_ConstantPool *cp)
{
    ...
    exe->constant_pool = MEM_realloc(exe->constant_pool,
                                     sizeof(DVM_ConstantPool) * (exe->constant_pool_count + 1));
...
copy_local_variables(FunctionDefinition *fd, int param_count)
{
    ...
    for (i = 0; i < local_variable_count; i++)
    {
        ...
        dest[i].type = copy_type_specifier(fd->local_variable[i + param_count]->type);
    }
...
copy_type_specifier(TypeSpecifier *src)
{
         ...
         derive = derive->next, i++)
    {
        ...
        case DVM_FUNCTION_DERIVE:
            ...
            dest->derive[i].u.function_d.parameter = copy_parameter_list(derive->u.function_d.parameter_list,
                                                                         &param_count);
...
add_global_variable(DKC_Compiler *compiler, DVM_Executable *exe)
{
    ...
    for (dl = compiler->declaration_list, i = 0; dl; dl = dl->next, i++)
    {
        ...
        exe->global_variable[i].type = copy_type_specifier(dl->declaration->type);
    }
}
...
add_line_number(OpcodeBuf *ob, int line_number, int start_pc)
{
    if (ob->line_number == NULL || (ob->line_number[ob->line_number_size - 1].line_number != line_number))
    {
        ob->line_number = MEM_realloc(ob->line_number,
                                      sizeof(DVM_LineNumber) * (ob->line_number_size + 1));
                               ...
                               Expression *expr, OpcodeBuf *ob)
{
    ...
    {
        generate_code(ob, expr->line_number,
                      DVM_PUSH_STACK_INT + get_opcode_type_offset(expr->u.identifier
                                                                      .u.declaration
                                                                      ->type->basic_type),
    ...
    {
        generate_code(ob, expr->line_number,
                      DVM_PUSH_STATIC_INT + get_opcode_type_offset(expr->u.identifier
                                                                       .u.declaration
                                                                       ->type->basic_type),
                      expr->u.identifier.u.declaration->variable_index);
    }
                           ...
                           OpcodeBuf *ob)
{
    if (decl->is_local)
    {
        generate_code(ob, line_number,
                      DVM_POP_STACK_INT + get_opcode_type_offset(decl->type->basic_type),
                      decl->variable_index);
    }
    else
    {
        generate_code(ob, line_number,
                      DVM_POP_STATIC_INT + get_opcode_type_offset(decl->type->basic_type),
                      decl->variable_index);
    }
}
                           ...
                           DVM_Boolean is_toplevel)
{
    ...
    case ADD_ASSIGN:
        generate_code(ob, expr->line_number,
                      DVM_ADD_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
    case SUB_ASSIGN:
        generate_code(ob, expr->line_number,
                      DVM_SUB_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
    case MUL_ASSIGN:
        generate_code(ob, expr->line_number,
                      DVM_MUL_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
    case DIV_ASSIGN:
        generate_code(ob, expr->line_number,
                      DVM_DIV_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
    case MOD_ASSIGN:
        generate_code(ob, expr->line_number,
                      DVM_MOD_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
    default:
...
generate_binary_expression(DVM_Executable *exe, Block *block,
                           ...
                           OpcodeBuf *ob)
{
    DBG_assert(expr->u.binary_expression.left->type->basic_type == expr->u.binary_expression.right->type->basic_type,
               ("left..%d, right..%d",
    ...
    generate_code(ob, expr->line_number,
                  code + get_opcode_type_offset(expr->u.binary_expression.left
                                                    ->type->basic_type));
}
                    ...
                    Expression *expr, OpcodeBuf *ob)
{
    ...
    case MINUS_EXPRESSION:
        ...
        generate_code(ob, expr->line_number,
                      DVM_MINUS_INT + get_opcode_type_offset(expr->type->basic_type));
        break;
                              ...
                              Expression *expr, OpcodeBuf *ob)
{
    ...
    else if (expr->kind == INCREMENT_EXPRESSION || expr->kind == DECREMENT_EXPRESSION)
    {
...
add_functions(DKC_Compiler *compiler, DVM_Executable *exe)
{
    ...
    for (fd = compiler->function_list, i = 0; fd; fd = fd->next, i++)
    {
        ...
        if (fd->block)
        {
            ...
            exe->function[i].need_stack_size = calc_need_stack_size(exe->function[i].code,
